...
import { RoomSessionManagerEventScenarioProgress } from '../room/events/RoomSessionManagerEventScenarioProgress';
import { RoomSessionManagerEventRoomPhysicsUpdated } from '../room/events/RoomSessionManagerEventRoomPhysicsUpdated';
import { MutableNode } from '../graph/MutableNode';
import { RoomSessionManagerEventRoomUpdated } from '../room/events/RoomSessionManagerEventRoomUpdated';
import { RoomStateData } from '../room/RoomStateData';
...
export class WebSocketsManager {
  ...
  public bootstrap(): void {
    ...
    io.on('connection', (s: Socket): void => {
      ...
      wsClient.onRoomChanged$.subscribe((room: string | null): void => {
        ...
        if (room != null) {
          const roomState: RoomState = this._roomSessionManager.getRoom(room);
  ...
  private _handleGrabNode(
    ...
    this._roomSessionManager.grabNode({
      nodeId: nodeId,
      roomId: roomId,
      userId: wsClient.id,
    });
  ...
  private _handleMoveNodes(
    ...
    this._roomSessionManager.moveNodes(roomId, m.nodes);
  }
  ...
  private _handleUngrabNode(
    ...
    this._roomSessionManager.ungrabNode({
      nodeId: m.nodeId,
      roomId: roomId,
      userId: wsClient.id,
    });

    this._roomSessionManager.saveRoom(roomId);
  }
  ...
  private _handleRoomUpdate(message: RoomSessionManagerEventRoomUpdated): void {
    const state: RoomState = this._roomSessionManager.getRoom(message.roomId);
    match(state)
