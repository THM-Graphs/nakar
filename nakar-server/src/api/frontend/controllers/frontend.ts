...
export default {
  ...
  getScenarioGroups: StrapiContext.handleRequest(async (context: StrapiContext): Promise<SchemaScenarioGroups> => {
  ...
  getScenarios: StrapiContext.handleRequest(async (context: StrapiContext): Promise<SchemaScenarios> => {
    const scenarioGroupId: string = context.getQueryParameter('scenarioGroupId');
