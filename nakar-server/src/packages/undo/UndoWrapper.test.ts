...
void test('transaction updates current state and exposes undo action', (): void => {
...
void test('undo and redo move between states', (): void => {
  const undo: UndoWrapper<TestState> = createUndoWrapper(5);
...
void test('snapshot captures state and is undoable', (): void => {
  const undo: UndoWrapper<TestState> = createUndoWrapper(5);
...
void test('maximum stack size trims oldest entries', (): void => {
...
void test('reset clears history stacks', (): void => {
  const undo: UndoWrapper<TestState> = createUndoWrapper(5);
