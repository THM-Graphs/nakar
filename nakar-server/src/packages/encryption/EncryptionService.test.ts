...
void describe('EncryptionService', (): void => {
  void describe('constructor', (): void => {
    void it('creates instance when config has a valid key that matches currentKeyId', (): void => {
      const service: EncryptionService = new EncryptionService({
        currentKeyId: 'key1',
        keys: [
          { id: 'key1', secret: 'V/dRbrjCVL5awGenc0goXCa4KR0xcvKUPZXdhQiyClQ=' },
        ],
      });

      assert.ok(service instanceof EncryptionService);
    });

    void it('throws NoKeysConfiguredError when keys array is empty', (): void => {
      assert.throws((): void => {
        new EncryptionService({ currentKeyId: 'key1', keys: [] });
  ...
  void describe('encrypt', (): void => {
    ...
    void it('throws UnknownKeyIdError when currentKeyId is not in the stored keys', (): void => {
      assert.throws((): void => {
        new EncryptionService({
          currentKeyId: 'key1',
          keys: [
            { id: 'key2', secret: 'V/dRbrjCVL5awGenc0goXCa4KR0xcvKUPZXdhQiyClQ=' },
          ],
        });
      }, UnknownKeyIdError);
    });
  ...
  void describe('decrypt', (): void => {
    ...
    void it('decrypts old payload after rotating currentKeyId to a newer key', (): void => {
      const oldService: EncryptionService = new EncryptionService({
        currentKeyId: 'key1',
        keys: [
          { id: 'key1', secret: 'V/dRbrjCVL5awGenc0goXCa4KR0xcvKUPZXdhQiyClQ=' },
          { id: 'key2', secret: 'ZyoXJKWlLtYvO9m28IfdABmTyRovn+MjHGr8H7Pb/F0=' },
        ],
      });

      const payload: EncryptedPayload = oldService.encrypt('Hello World');

      const rotatedService: EncryptionService = new EncryptionService({
        currentKeyId: 'key2',
        keys: [
          { id: 'key1', secret: 'V/dRbrjCVL5awGenc0goXCa4KR0xcvKUPZXdhQiyClQ=' },
          { id: 'key2', secret: 'ZyoXJKWlLtYvO9m28IfdABmTyRovn+MjHGr8H7Pb/F0=' },
